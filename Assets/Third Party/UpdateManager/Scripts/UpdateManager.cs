...
public class UpdateManager : MonoBehaviour
{
	...
	void Start()
	{
		UpdateManager[] updateManager = FindObjectsOfType<UpdateManager>();

		if(updateManager.Length > 1)
		{
			Destroy(gameObject);
		}
	...
	public static void RemoveSpecificItemAndDestroyIt(OverridableMonoBehaviour behaviour)
	{
		...
		if (behaviour.photonView != null)
		{
			PhotonNetwork.Destroy(behaviour.gameObject);
		}
		else
		{
	...
	private void Update()
	{
		if (count > 0)
		{
			...
			{
				if (array[i] == null || array[i].enabled == false)
				{
					continue;
				}
				array[i].UpdateMe();
